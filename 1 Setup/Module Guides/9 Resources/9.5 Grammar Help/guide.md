...
- **Key types:**
  ...
  - `GrammarHelpCoordinator` — receives context-lookup requests from `InterPanelRouter` (2.1) originating from `.editor`, `.markdownPreview`, or `.htmlPreview` sources; maps the query word to the best grammar topic (using `searchTerms` for fuzzy matches); opens/creates a tab for the result <!-- assumed -->
