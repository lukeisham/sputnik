...
| Aspect | Mode 1: Menu / Toolbar | Mode 2: Editor Lookup | Mode 3: Markdown Preview Lookup | Mode 4: HTML Preview Lookup |
|---|---|---|---|---|
| **Trigger** | Menu `Help > Grammar Help`, toolbar `?` button, or keyboard shortcut | Right-click context menu on a word in Text Editor (any mode — 3.1, 3.2, 3.3, 3.4, 3.5) | Right-click context menu on a word in Markdown Preview (Module 4) — the rendered preview captures the underlying text | Right-click context menu on selected text in HTML Preview (Module 8) — `WKWebView` selection is captured via injected `WKUserScript` and cached in the coordinator's `capturedSelection` |
| **Panel action** | Opens the Grammar Help panel (if closed) or raises it (if open), with the last-viewed topic or the topic index as the initial tab | Opens or raises the panel and creates a **new tab** for the matched topic; does not replace existing tabs | Same as editor lookup — opens/raises panel and creates a new tab for the matched topic | Same — opens/raises panel and creates a new tab for the matched topic |
| **Navigation** | Free — browse sidebar topics or search | Automatic — panel jumps to the best-matching grammar topic for the selected word | Same — jumps to the best-matching topic | Same — jumps to the best-matching topic |
...
| **Panel state** | Remembers open tabs and active tab across session restarts via `PersistenceService` (2.5) | Tabs persist; closing the panel collapses it but preserves its tab state | Same | Same |
| **Source dispatch** | N/A — direct open | `.editor` source via `SputnikHelpContextResolver.resolve()` | `.markdownPreview` source via `SputnikHelpContextResolver.resolve()` | `.htmlPreview` source via `SputnikHelpContextResolver.resolve()` |
...
- **Key types:**
  ...
  - `GrammarHelpCoordinator` — receives context-lookup requests from `.editor`, `.markdownPreview`, or `.htmlPreview` sources; maps the query word to the best grammar topic (using `searchTerms` for fuzzy matches); exposes `var onNavigate: ((HelpRequest) -> Void)?` — wired by `GrammarHelpPanelView.task` to write `AppState.requestedHelpTarget` (weakly captured); `openHelp(for:)` calls `onNavigate` instead of posting a `NotificationCenter` notification (ISS-008 resolved)
    - `GrammarHelpSource` — enum with cases `.editor`, `.markdownPreview`, and `.htmlPreview`; the source is passed through the `SputnikHelpContextResolver` from the host panel so the coordinator knows which origin triggered the lookup
...
- **Dependencies:** Foundation 2.1 InterPanelRouter (context lookup from `.editor`, `.markdownPreview`, and `.htmlPreview` sources, plus panel open/raise routing); Foundation 2.4 UI/UX (panel slot registration, tab bar chrome); Foundation 2.5 PersistenceService (tab state persistence); Foundation 2.7 (`MoreContextMenu`, `HelpContextResolving`, `HelpContextQuery` — the shared builder and query types through which all three host panels dispatch lookups); Text Editor 3.5 (spelling/grammar checker integration, right-click trigger); Markdown Preview 4 (right-click trigger source for Mode 3); HTML Preview 8 (right-click trigger source for Mode 4).
- **Failure modes:**
