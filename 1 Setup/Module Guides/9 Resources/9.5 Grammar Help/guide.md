...
- **Data flow:**
  ...
  - *Editor lookup (Mode 2):* User right-clicks a word in Text Editor (any mode) → context menu shows "Grammar: <word>" → `InterPanelRouter` sends `{source: .editor, query: <word>, fileType: <current-mode>, cursorContext: <sentence>}` → `GrammarHelpCoordinator` searches the index by `title` and `searchTerms` → calls `GrammarHelpPanelViewModel.openTab(topicID:)` which opens/raises the panel and activates/creates the tab. Falls back to search results with query pre-filled if no match.
