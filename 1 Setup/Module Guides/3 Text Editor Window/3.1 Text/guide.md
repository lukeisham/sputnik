...
| `EditorTextView.swift` | `NSTextView` subclass — key handling (Tab for ghost text, ⌘F for find), single-click for quickfix popover, drag delegate for file drops; holds weak refs to ghost overlay, search, spelling checker, editor VM, help resolver |
| `EditorViewModel.swift` | `@MainActor @Observable` — file URL, dirty state, undo manager, `EditorMode`, gating flags (`htmlModeActive`, `spellCheckActive`), loaded text, load token, search controller, text view reference, file watcher, mode inference |
...
- **State owned:** open file URL, `NSTextStorage` contents, undo/redo stack, dirty flag, crash-recovery cache path, search match list, current `EditorMode`, gating flags (`htmlModeActive`, `spellCheckActive`)
