...
| File | Responsibility |
|---|---|
| `SpellCheckFileTypeGuard.swift` | `enum` — gates spell/grammar checking to `.txt` and `.md`; sets `EditorViewModel.spellCheckActive` on file open |
| `SpellingGrammarChecker.swift` | `@MainActor` — wraps `NSSpellChecker`; writes red (spelling) and orange (grammar) underlines to `NSTextStorage`; maintains `[GrammarAnnotation]` model with spelling-over-grammar priority |
| `GrammarAnnotation.swift` | `Sendable` value type — `range`, `kind` (spelling/grammar), `suggestions`, `isSuppressed` (grammar hidden under spelling) |
| `QuickfixPopover.swift` | SwiftUI popover — Fix (applies suggestion) / Dismiss (ignores word/phrase + re-checks) |
| `QuickfixPresenter.swift` | `@MainActor` — right-click `NSMenu` corrections presenter; superseded by single-click `QuickfixPopover` but retained as a component |

## Technical Summary
- **Framework(s):** AppKit (`NSSpellChecker`, `NSTextCheckingResult`, `NSTextView`, `NSTextStorage`), Foundation
- **Key types:**
  - `SpellCheckFileTypeGuard` — `enum`; checks the open file's extension on file open; enables the checker for `.txt` and `.md`; disables it for all other types (`.html`, binary, etc.); result sets `EditorViewModel.spellCheckActive`
  - `SpellingGrammarChecker` — `@MainActor` class; subscribes to `NSTextStorage` change notifications; calls `NSSpellChecker.check(_:range:types:options:inSpellDocumentWithTag:orthography:wordCount:)` after debounce; writes `.spelling` results as **red** underlines and `.grammar` results as **orange** underlines to `NSTextStorage`; rebuilds a parallel `[GrammarAnnotation]` model on each pass; applies **spelling-over-grammar** priority (a grammar annotation overlapping a spelling range is kept but `isSuppressed`, so it is not drawn); maintains a per-document dismissed set (spelling via `NSSpellChecker.ignoreWord`, grammar via a local phrase set); only runs when `spellCheckActive` is `true`
  ...
  - `EditorTextView` (3.1) — `NSTextView` subclass; single left-click hit-tests the annotation model and presents `QuickfixPopover`; `menu(for:)` appends a "Look Up in … Help" item for the active mode, resolves the selection through the matching module-9 coordinator, and routes the result via `AppState.requestedHelpTarget` (Foundation `HelpRequest`)
  - `QuickfixPresenter` — `@MainActor` class; earlier right-click correction `NSMenu`; superseded for the primary flow by the single-click `QuickfixPopover`, retained as a component
...
## Invariants
- `SpellingGrammarChecker` is `@MainActor` — all `NSSpellChecker` calls and `NSTextStorage` attribute mutations happen on the main actor (SW-1, AppKit requirement)
- `SpellCheckFileTypeGuard.check(_:viewModel:)` is called **once** at file open — the flag persists for the document lifetime
- Spelling-over-grammar priority: grammar annotations overlapping a spelling range are kept in the model but marked `isSuppressed` (never drawn) — fixing or dismissing the spelling issue triggers a re-check that surfaces the grammar underline (SC-12)
- `NSSpellChecker` is the **only** spell checker used — no third-party or custom spell-checking (SR-5)
- `QuickfixPopover` is a SwiftUI view hosted in `NSPopover` (SW-3 — minimal AppKit seam)
