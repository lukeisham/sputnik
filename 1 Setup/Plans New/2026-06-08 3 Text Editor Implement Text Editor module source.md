...
- [x] 1. **Editor state core — `EditorMode` + `EditorViewModel`**
...
- [x] 2. **File-load protection — `EncodingGuard`**
...
- [x] 3. **Text surface — `EditorTextView` + `LineNumberRulerView`**
...
- [x] 5. **Shared editor utilities — `SyntaxHighlighter` + `GhostTextOverlay`**
...
- [x] 6. **External-change watching — `FileWatcher`**
...
- [x] 7. **Auto-save + crash recovery — `CrashRecoveryStore`**
...
- [x] 8. **Find/replace — `SearchController` + `SearchBarView`**
...
- [x] 9. **Markdown tier (3.2) — `MarkdownLanguageProvider`**
...
- [x] 10. **HTML tier (3.4) — guard, provider, render command**
...
- [x] 11. **Spelling/grammar tier (3.5) — guard, checker, quickfix**
...
- [x] 12. **ASCII tier 1 — auto box-drawing (`ASCIIArtLanguageProvider` + `BlockCompletion`)**
...
- [x] 13. **ASCII tier 2 — Studio panel (`ImageToASCIIConverter`, `ASCIILibraryBrowser`, `ASCIIStudioPanel` + `ASCIIStudioView`)**
...
- [x] 14. **Cross-file consistency + rules audit**
...
## Closeout
- [x] Re-read the Purpose statement — does the outcome match it exactly? ✅ Outcome matches: all 25 Swift source files for module 3 (3.1–3.5) authored; shared utilities in 3.1; Foundation seams referenced not implemented; local defaults shipped for ISS-004 gaps.
- [x] Success Condition verified (Step 14 inspection pass — all 10 points confirmed) ✅ SC-1 through SC-10 all pass.
- [x] ISS-004 reviewed — left Open with a workaround note in Issues.md detailing the local defaults shipped at each seam; real fix belongs in Foundation 2.3.
- [x] Module Guides 3.1–3.5 updated (`status: active`, `last_updated: 2026-06-08`).
- [x] Changes committed and pushed: staged all modifications (Text Editor source, Foundation follow-ups, HTML Preview docs, updated guides, Issues.md) plus new untracked files and pushed to `origin/main` — cumulative push reflecting all recent changes (Foundation + Text Editor + HTML Preview + guides) in a single batch.
