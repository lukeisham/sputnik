...
extension SputnikAlert {
    ...
    public var message: String {
        ...
        case .unsupportedFileType(let url):
            return
                "\"\(url.lastPathComponent)\" cannot be opened in Sputnik. Binary or unknown file types are not supported."
        ...
        case .layoutRestoreFailed:
            return
                "The saved window layout could not be restored and has been reset to the default arrangement."
        case .recoveryAvailable(let name):
            return
                "Sputnik found unsaved changes for \"\(name)\" from a previous session. Would you like to recover them?"
        case .custom(_, let m):
            return m
        }
    }
